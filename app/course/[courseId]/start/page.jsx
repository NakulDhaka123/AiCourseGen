...
const CourseStart = ({ params }) => {
  ...
  return (
    <div className="p-2">
      ...
      <div className={`fixed md:w-64 w-64 h-screen border-r shadow-sm z-20 transform transition-transform md:translate-x-0 ${
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        } md:block`}>
        <h2 className="rounded-xl font-medium text-lg bg-primary p-3 text-white">
          {course?.courseOutput?.course?.name}
        </h2>
        
        <div className="h-[calc(100%-3rem)] rounded-xl overflow-y-auto">
          {course?.courseOutput?.course?.chapters.map((chapter, index) => (
            <div
              className={`cursor-pointer rounded-xl hover:bg-purple-50 ${
