  ...
  return (
    <div className="flex justify-between items-center gap-2 p-5 shadow-sm">
      <div className="flex items-center gap-2">
        ...
        <span className="font-bold text-xl">CodeCatalysts</span>
           </Link>
      ...
      <div className="md:hidden">
        ...
        {menu.map((item) => (
           item.isLogout ? (
            <li
            key={item.id}
            className={`flex items-center gap-2 text-white cursor-pointer p-3 hover:bg-gray-100 hover:text-white rounded-lg mb-3`}
          ...
          <Link href={item.path}>
                <DropdownMenuItem>
                  <li
                    key={item.id}
                    className={`flex text-xs items-center gap-1 text-white cursor-pointer p-3 hover:bg-gray-100 hover:text-white rounded-lg mb-3 ${
                      item.path == path && "bg-gray-100 text-white"
                    }`}
                  >
                    <div>{item.icon}</div>
                    <h2>{item.name}</h2>
